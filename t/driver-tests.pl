...
sub count_group_by : Tests(34) {
    ...
    my $cgb_iter4 = Bar->count_group_by(
        ...
        {   group => ['extract(month from created_on)'],
            sort  => [
                {   column => 'extract(month from created_on)',
                    desc   => 'desc'
                }
            ]
        }
    ...
    # Sort by count
    my $cgb_iter5 = Bar->count_group_by(
        undef,
        {   group => ['foo_id'],
            sort  => 'cnt',
        }
...
sub max_group_by : Tests(7) {
    my $mgb = Bar->max_group_by(
        undef,
        {   join  => Foo->join_on( undef, { 'id' => \'=bar_foo_id', } ),
            group => ['foo_id'],
            max   => 'created_on',
        }
    );
...
sub joins : Tests(1) {
    ...
    my @data  = Foo->load(
        ...
        {   joins => [
                [   'Baz', undef,
                    { bar_id => \'= bar1.bar_id', status => 3 }, {}
                ],
                [   'Bar', 'foo_id',
                    { status => 3 }, { alias => 'bar1', to => 'foo' }
                ],
                [   'Bar', undef,
                    { foo_id => \'= bar1.bar_foo_id', status => 4 },
                    { alias  => 'bar2' }
                ]
            ],
            sort      => 'created_on',
    ...
    are_objects( \@data, [$vista],
        'Has bar_status = 3, bar_status = 4, baz_status = 3 (only joins)' );
...
sub count_with_joins : Tests(2) {
    ...
    my $count_args1 = {
        joins => [
            [   'Baz', undef,
                { bar_id => \'= bar_id', status => 1 }, { unique => 1 }
            ],
        ],
    };
    my $count_args2 = {
    ...
    is( Bar->count( undef, $count_args1 ),
        2, 'count method must looks up unique options of joins.' );
    my @bars = Bar->load( undef, $count_args2 );
    is( Bar->count( undef, $count_args3 ),
        scalar @bars,
        'count method must returns same as number of objects that load method returns.'
    );
}
...
sub joins_with_join : Tests(1) {
    ...
    are_objects( \@data, [$original],
        'Has bar_status = 3, bar_status = 4, baz_status = 3 (joins with join)'
    );
}
...
sub only_join : Tests(1) {
    ...
    are_objects( \@data, [$original],
        'Has bar_status = 3, bar_status = 4, baz_status = 3 (only join)' );
}
...
sub alias : Tests(3) {
    ...
    @a_foos = Foo->load(
        ...
        {   join => [
                ...
                {   join => [
                        'Bar', undef,
                        { foo_id => \'= bar1.bar_foo_id', status => 3 },
                        { alias  => 'bar2' }
                    ],
                    alias => 'bar1'
                }
    ...
    @a_foos = Foo->load(
        ...
        {   join => [
                ...
                {   join => [
                        'Bar', undef,
                        { foo_id => \'= bar1.bar_foo_id', status => 0 },
                        { alias  => 'bar2' }
                    ],
                    alias => 'bar1'
                }
...
sub sock_no_class : Tests(9) {
    ...
    my @socks = Sock->load( { text => 'ABC', }, { no_class => 1, } );
    is( scalar @socks, 2, 'Search for all Socks finds a pair of socks' );
    @socks = sort { ref($a) cmp ref($b) } @socks;
    is( ref $socks[0], 'Sock::Fish',
        'One of the discovered Socks is a fish' );
...
sub sock_array_no_class_terms : Tests(12) {
    ...
    is( ref $socks[1],
        'Sock::Monkey', 'The other discovered Sock is a monkey' );
    is( ref $socks[2],
        'Sock::Monkey', 'The other discovered Sock is a monkey (2)' );
}
...
sub left_join_multi_joins : Tests(2) {
    ...
    my @data = Foo->load(
        ...
        {   joins => [
                ...
                [   'Baz', undef,
                    { bar_id => \'is null', },
                    { type   => 'left', condition => 'bar_id', to => 'bar' }
                ],
            ],
            sort => 'id',
        }
...
@tmp = Foo->load(
    ...
    {   sort      => 'created_on',
        ...
        join      => [
            'Bar', undef,
            { foo_id => \'= foo_id', status => 0 }, { unique => 1 }
        ]
    }
);
