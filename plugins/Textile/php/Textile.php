...
class Textile
{
    ...
    public function _save_link($matches)
    {
        $this->links[$matches[1]] = array('url' => $matches[3],
            'title' => $matches[2]);
        return $matches[4];
    }
    public function _replace_image($matches)
    {
    ...
    public function _replace_span($matches)
    {
    ...
    public function TextileThis($str)
    {
            ...
            if ($block == 'bc') {
                ...
                $para = preg_replace_callback(
                    "/(?:^|(?<=[\s>])|([{[]))
						   ==(.+?)==
						   (?:$|([\]}])|(?=[[:punct:]]{1,2}|\s))/sx",
                          array(&$this, '_replace_inline_block'),
                    $para
                );
    ...
    public function format_paragraph($args)
    {
        $save_repl = $this->repl;
        $this->repl = array();
    ...
    public function format_inline($args)
    {
        ...
        $text = preg_replace_callback(
            '/\b([A-Z][A-Za-z0-9]*?[A-Z0-9]+?)\b(?:[(]([^)]*)[)])/',
                  array(&$this, '_replace_acronym'),
            $text
        );
    ...
    public function format_macro($args)
    {
        $macro = $args['macro'];
    ...
    public function format_cite($args)
    {
    ...
    public function format_code($args)
    {
    ...
    public function format_classstyle($clsty, $class = '', $style = '')
    {
        $class = preg_replace('/^ /', '', $class);
    ...
    public function format_tag($args)
    {
        $tagname = $args['tag'];
        $text = array_key_exists('text', $args) ? $args['text'] : '';
        $pre = array_key_exists('pre', $args) ? $args['pre'] : '';
    ...
    public function _add_term($dt, $dd)
    {
    ...
    public function format_deflist($args)
    {
    ...
    public function format_list($args)
    {
        $str = array_key_exists('text', $args) ? $args['text'] : '';
    ...
    public function format_block($args)
    {
        $str = array_key_exists('text', $args) ? $args['text'] : '';
        $inline = $args['inline'];
        $pre = array_key_exists('pre', $args) ? $args['pre'] : '';
    ...
    public function format_link($args)
    {
        $text = array_key_exists('text', $args) ? $args['text'] : '';
    ...
    public function format_url($args)
    {
    ...
    public function format_span($args)
    {
        $text = array_key_exists('text', $args) ? $args['text'] : '';
        $pre = array_key_exists('pre', $args) ? $args['pre'] : '';
        $post = array_key_exists('post', $args) ? $args['post'] : '';
    ...
    public function format_image($args)
    {
    ...
    public function format_table($args)
    {
        $str = array_key_exists('text', $args) ? $args['text'] : '';
    ...
    public function encode_html($html, $can_double_encode = 0)
    {
        if (!isset($html)) {
            return '';
        }
    ...
    public function decode_html($html)
    {
    ...
    public function encode_html_basic($html, $can_double_encode = 0)
    {
    ...
    public function image_size($file)
    {
        if ($Have_ImageSize) {
            if (file_exists($file)) {
    ...
    public function mail_encode($addr)
    {
    ...
    public function process_quotes($str)
    {
        return $str;
    }
    ...
    public function default_macros()
    {
