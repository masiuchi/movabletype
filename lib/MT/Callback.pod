...
Callback.pod - MyMTOS Callbacks

=head1 DESCRIPTION

The callback system in MyMTOS is easy to use and is invoked at key areas
throughout the application. This document is a reference of all available
callbacks in MyMTOS itself.
...
The MyMTOS publishing callbacks are invoked from the
...
When a new weblog is created, MyMTOS uses the I<MT::DefaultTemplates>
