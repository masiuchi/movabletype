...
sub start_element {
    ...
    return
        unless MT::BackupRestore::NS_MOVABLETYPE() eq $ns
    ...
    if ( $name eq 'author' && $self->{digest_sha_not_found} ) {
        ...
        if ( $pass =~ m/^\$6\$/ ) {
            die MT->translate(
                "Cannot restore requested file because doing so requires the Digest::SHA Perl language module. Please contact your MyMTOS system administrator."
            );
        }
    }
